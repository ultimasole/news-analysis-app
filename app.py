...
def analyze_sentiment(title, snippet, client):
    ...
    prompt = f"""
    1)당신은 PR 전문가다. 다음 뉴스를 분석해 [긍정], [부정], [중립] 중 어느 감성에 해당하는지 분류하고 이유를 설명하라.
    2)원칙적으로, 아무 제한 없이 제목 자체의 감성을 분류하라. 
    3)예외적으로, 뉴스 제목에 현대카드 또는 그 경쟁사가 언급된 경우엔 현대카드를 향한 감성이 무엇인지를 기준으로 분류하라.
    4)반드시 첫 단어를 [분류]로 시작하라. 예: [긍정] 실적 발표 수치가 시장 기대치를 상회함.
